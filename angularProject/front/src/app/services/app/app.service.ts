...
export class AppService {
    ...
    private ohillDone = false;
    private newcombDone = false;
    private runkDone = false;
    ...
    setData(): void {
                ...
                catchError((err, caught) => {
                    ...
                    setTimeout(() => {
                        this._dataLoaded.next(
                            this.newcombDone && this.ohillDone && this.runkDone,
                        );
                    }, 400);
                    throw err;
                }),
        ...
        this.call('runk')
            ...
            .subscribe((data) => {
                this.runk = data;
                this.setHeaders('runk');
            });
    }

    private call(target: string) {
        return this.http
            .get(`http://localhost:4200/api/${target}`)
    ...
    private setHeaders(target: string): void {
        const diningHall = this.headerHelper(target);
        if (diningHall === '?????') {
            return;
        }
    ...
    private setShopToItem(target: string): void {
        const l = this.shopHelper(target).values();

        for (const elem of l) {
            //load only 4 items, before then moving on (lazy load others) ; also add loading animation
            switch (target) {
                case 'ohill': {
        ...
        for (const key in diningHall) {
            if (target === 'runk') {
                if (diningHall[key].stationName in this.runkShopToItems) {
                    this.runkShopToItems[diningHall[key].stationName].push(
                        diningHall[key].item,
                    );
                }
            } else if (target === 'ohill') {
                if (diningHall[key].stationName in this.ohillShopToItems) {
                    this.ohillShopToItems[diningHall[key].stationName].push(
                        diningHall[key].item,
                    );
                }
            } else if (target === 'newcomb') {
                if (diningHall[key].stationName in this.newcombShopToItems) {
                    this.newcombShopToItems[diningHall[key].stationName].push(
                        diningHall[key].item,
                    );
                }
            }
            ...
            switch (target) {
                case 'ohill': {
                    for (const elem of Object.keys(this.ohillShopToItems)) {
                        this.ohillShopToItems[elem].sort(function (
                            a: itemObject,
                            b: itemObject,
                        ) {
                            return (
                                b.itemName.length +
                                b.itemDesc.length -
                                (a.itemName.length + a.itemDesc.length)
                            );
                        });
                    }
                    this.ohillDone = true;
