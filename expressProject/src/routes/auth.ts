...
import { ROUTES, type AuthConfirmOutput } from "hoorank-shared";
...
import { setSessionCookie, setCSRFCookie } from "./utils.js";

export const authRouter = Router();
authRouter.get(ROUTES.AUTH.REFRESH, async (req: IUserRequest, res): Promise<void> => {
    if (req.cookies.CSRF_TOKEN && req.signedCookies.SESSION_ID) {
        ...
        const { sessionId, newCsrfToken, user } = result;
