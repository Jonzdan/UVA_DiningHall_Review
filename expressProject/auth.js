...
async function csrf(req, res, next) {
    if (req.headers.h_csrf_token && req.headers.h_csrf_token == req.cookies.CSRF_TOKEN) {
        const result = await identifierSchema.find({
            csrf_token: req.headers.h_csrf_token,
        });
        if (result === undefined || result.length === 0) {
            res.status(400).json({
                msg: "Unidentified CSRF Token",
            }).end();
            return;
        }
        else {
        ...
        res.status(400).end("Failed Validation");
        return;
    }
}
...
async function validateFields(req, res, next) {
    ...
    if (Object.keys(req.body).length === 4) {
        if (!email || ! username || !secondPass || !firstPass) {
            res.status(400).json({
                msg: "Invalid Input"
            }).end();
            return;
        ...
        else if (firstPass.length > 32) {
            codes.push("1000");
        }
        if (secondPass.length < 8 ) {
            codes.push("2001");
        }
        else if (secondPass.length > 32) {
            codes.push("2000");
        }
...
async function loggedIn_Or_Not(req, res, next) {
    ...
    if (response && response.length === 1 && response[0].userID !== undefined) {
        res.status(400).json({
